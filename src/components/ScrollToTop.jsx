...
function ScrollToTop({ hidden }) {
  ...
  return (
    <button
      ...
      className={`fixed bottom-24 right-6 z-40 flex h-11 w-11 items-center justify-center rounded-full border border-slate-700 bg-slate-900/80 text-slate-300 shadow-lg backdrop-blur-md transition-all duration-300 hover:border-cyan-500/50 hover:text-cyan-400 light:border-slate-200 light:bg-white/80 light:text-slate-600 light:hover:border-cyan-500/50 light:hover:text-cyan-600 ${
        isVisible && !hidden ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-4 opacity-0'
      }`}
    >
      <ArrowUp size={18} />
    </button>
